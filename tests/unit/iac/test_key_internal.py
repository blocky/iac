...
def test_create_key_pair_file__happy_path(
    mock_key_file_name,
    mock_open,
    mock_exists,
    aws_parrot,
    ...
    mock_key_file_name.return_value = "key-file-name"
    mock_exists.return_value = False
    mock_open.return_value.__enter__.return_value = file
    file.write.return_value = None

    iac.key.create_key_pair_file(aws_parrot.key_name, key_material)

    mock_key_file_name.assert_called_once_with(aws_parrot.key_name)
...
@patch("builtins.open")
@patch("iac.key.key_file_name")
def test_create_key_pair_file__file_already_exists(
    mock_key_file_name,
    mock_open,
    mock_exists,
    aws_parrot,
    ...
    with raises(IACKeyError) as exc_info:
        iac.key.create_key_pair_file(aws_parrot.key_name, None)
...
@patch("builtins.open")
@patch("iac.key.key_file_name")
def test_create_key_pair_file__write_fails(
    mock_key_file_name,
    mock_open,
    mock_exists,
    aws_parrot,
...
@patch("os.remove")
@patch("iac.key.key_file_name")
def test_delete_key_pair_file__happy_path(
    mock_key_file_name,
    mock_remove,
    aws_parrot,
):
    key_file_name = "key-file-name"
    mock_key_file_name.return_value = key_file_name

    iac.key.delete_key_pair_file(aws_parrot.key_name)

    mock_key_file_name.assert_called_once_with(aws_parrot.key_name)
    mock_remove.assert_called_once_with(key_file_name)


@patch("os.remove")
@patch("iac.key.key_file_name")
def test_delete_key_pair_file__remove_fails(
    mock_key_file_name,
    mock_remove,
    aws_parrot,
):
